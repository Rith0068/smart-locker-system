...
<div class="w-full px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form action="{{ route('locker.update', $locker->id) }}"
          method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="locations_id" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none">
                    @foreach ($locations as $location)
                        <option value="{{ $location->id }}" @selected(old('locations_id', $locker->locations_id) == $location->id)>{{ $location->name_location }}</option>
                    @endforeach
                </select>
